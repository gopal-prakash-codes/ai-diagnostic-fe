...
import { ArrowLeft, Stethoscope, LogOut, User, ChevronLeft, ChevronRight } from 'lucide-react';

// Reusing your existing SVG components with Lucide React for better integration
// Assuming 'lucide-react' is installed and used consistently. If not, stick to your SVG components.
// I've replaced your custom SVG components with the Lucide React equivalent for better code practice.
// If you don't want to add lucide-react, simply change these back to your original SVG components.
...
function PatientHistory() {
  ...
  // Filter diagnoses based on selectedDate
  const filteredDiagnoses = historyData?.diagnoses?.filter(diagnosis => {
    if (!selectedDate) {
      return true; // If no date is selected, show all
    }
    const diagnosisDate = new Date(diagnosis.createdAt).toISOString().split('T')[0];
    return diagnosisDate === selectedDate;
  }) || [];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          <label htmlFor="history-date" className="text-sm font-medium text-gray-700">Filter by Date:</label>
          <Input
            id="history-date"
            type="date"
            value={selectedDate}
          ...
          />
          {selectedDate && (
            <Button onClick={handleClearDate} variant="outline" size="sm" className="px-3 py-1 text-xs">
              Clear
            </Button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                Diagnosis History ({filteredDiagnoses.length} total)
              </h3>
            </div>
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-sm text-gray-500">This patient doesn't have any diagnosis history for the selected date.</p>
            </CardContent>
          </Card>
        )}
      </div>
